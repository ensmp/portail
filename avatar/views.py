...
def add(request, extra_context=None, next_override=None,
    ...
    if request.method == "POST" and 'avatar' in request.FILES:
        if upload_avatar_form.is_valid():
            ...
            avatar.save()
            messages.add_message(request, messages.INFO, "Votre photo a été envoyée avec succès")
...
def delete(request, extra_context=None, next_override=None, *args, **kwargs):
    ...
    if request.method == 'POST':
        if delete_avatar_form.is_valid():
            ...
            messages.add_message(request, messages.INFO, "Les photos ont été supprimées avec succès")
            return HttpResponseRedirect(next_override or _get_next(request))
    return render_to_response(
...
def render_primary(request, extra_context={}, user=None, size=AVATAR_DEFAULT_SIZE, *args, **kwargs):
    ...
    if avatar:
        ...
        url = get_default_avatar_url(request.user)
